...
export function extractVideoId(value) {
  ...
  try {
    ...
    else if (host === 'youtube.com' || host.endsWith('.youtube.com') || host === 'www.youtube-nocookie.com') {
      id =
        url.searchParams.get('v') ||
        (/^\/(shorts|embed|live|v)\//.test(url.pathname) ? url.pathname.split('/')[2] : '');
    } else id = '';
  } catch (_) {
...
export function selectVideoFormat(formats, quality = '1080p', preferences = {}, options = {}) {
  ...
  if (parsedQuality.kind === 'resolution') {
    candidates = resolvedFormats.filter(({ resolutionValue }) =>
      options.fallbackToLower ? resolutionValue <= parsedQuality.value : resolutionValue === parsedQuality.value
    );
    if (options.fallbackToLower && !candidates.length && resolvedFormats.length) {
      const minimum = Math.min(...resolvedFormats.map((f) => f.resolutionValue));
      candidates = resolvedFormats.filter((f) => f.resolutionValue === minimum);
    }
    if (options.fallbackToLower && candidates.length) {
      const best = Math.max(...candidates.map((f) => f.resolutionValue));
      candidates = candidates.filter((f) => f.resolutionValue === best);
    }
