...
function session(mediaSize, track = 'video') {
  ...
  sabr.partialSegmentQueue.set(1, {
    ...
    segmentNumber: 0,
    durationMs: '1000',
    mediaHeader: { contentLength: String(mediaSize) },
    bufferedChunks: [],
  });
  return sabr;
}
...
test('a paused video request does not block the independent audio request', async () => {
  ...
  const audioWork = audio.processStreamingResponse(response(512 * 1024).value).then(() => audio.outputs.audio.close());
...
test('protocol headers advance segment state and duplicate segments are not emitted twice', async () => {
  ...
  writer.write(
    ...
    FormatInitializationMetadata.encode({
      formatId: { itag: 137 },
      mimeType: 'video/mp4',
      durationUnits: '1000',
  ...
  for (const sequenceNumber of [0, 1, 1]) {
    writer.write(
      ...
      MediaHeader.encode({
        ...
        itag: 137,
        sequenceNumber,
        isInitSeg: sequenceNumber === 0,
        durationMs: sequenceNumber === 0 ? '0' : '1000',
        contentLength: '3',
      }).finish()
    );
