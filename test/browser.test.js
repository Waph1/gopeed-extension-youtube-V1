...
const source = readFileSync(new URL('../src/lib/browser.js', import.meta.url), 'utf8').replace(/^export /gm, '');
...
test('desktop conversion preserves Android Chrome and WebKit versions across upgrades', () => {
  ...
  for (const version of ['151.0.7922.200', '152.1.1234.56']) {
    const native =
      'Mozilla/5.0 (Linux; Android 16; 24069RA21C Build/BP2A.250605.031.A3; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/' +
      version +
      ' Mobile Safari/537.36';
    assert.equal(
      scope.desktopUserAgent(native),
...
test('iOS conversion keeps WebKit/Safari instead of introducing Chrome', () => {
  ...
  vm.createContext(scope);
  vm.runInContext(source, scope);
  const ua = scope.desktopUserAgent(
    'Mozilla/5.0 (iPad; CPU OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.0 Mobile/15E148 Safari/604.1'
  );
  assert.equal(
    ua,
    'Mozilla/5.0 (Macintosh; Intel Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.0 Safari/604.1'
  );
